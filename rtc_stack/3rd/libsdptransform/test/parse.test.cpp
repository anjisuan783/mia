...
SCENARIO("ChromeSdp1", "[parse]")
{
	auto sdp = helpers::readFile("test/data/chrome1.sdp");
	auto session = sdptransform::parse(sdp);

  //REQUIRE(session.at("version") == 2);
  
	REQUIRE(session.at("msidSemantic").at("semantic") == "WMS");
	REQUIRE(session.at("msidSemantic").find("token") == session.at("msidSemantic").end());
}

SCENARIO("ChromeSdp2", "[parse]")
{
	auto sdp = helpers::readFile("test/data/chrome2.sdp");
	auto session = sdptransform::parse(sdp);
	REQUIRE(session.at("msidSemantic").at("semantic") == "WMS");
	REQUIRE(session.at("msidSemantic").find("token") != session.at("msidSemantic").end());
	REQUIRE(session.at("msidSemantic").at("token") == "11 12 13");
}
