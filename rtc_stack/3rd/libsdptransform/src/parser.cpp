...
namespace sdptransform
{
	...
	{
		static const Regular::regex ValidLineRegex("^([a-z])=(.*)");
		...
		while (std::getline(sdpstream, line, '\n'))
		{
			...
			if (!Regular::regex_search(line, ValidLineRegex))
				continue;
			...
			for (size_t j = 0; j < rules.size(); ++j)
			{
				...
				if (Regular::regex_search(content, rule.reg))
				{
	...
	void parseReg(const grammar::Rule& rule, json& location, const std::string& content)
	{
		...
		Regular::smatch match;

		Regular::regex_search(content, match, rule.reg);
	...
	void insertParam(json& o, const std::string& str)
	{
		static const Regular::regex KeyValueRegex("^\\s*([^= ]+)(?:\\s*=\\s*([^ ]+))?$");
		...
		Regular::regex_match(str, match, KeyValueRegex);
