...
namespace erizo {
  ...
  SdpInfo::SdpInfo(const std::vector<RtpMap>& rtp_mappings) 
    : internalPayloadVector_{rtp_mappings} {
    isBundle = false;
    isRtcpMux = false;
    isFingerprint = false;
    dtlsRole = ACTPASS;
  ...
  bool SdpInfo::isValidExtension(const std::string& uri) {
    auto value = std::find_if(
        supported_ext_map_.begin(), supported_ext_map_.end(), [uri](const ExtMap &extension) {
