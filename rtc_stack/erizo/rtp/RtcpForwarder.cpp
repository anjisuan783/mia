...
void RtcpForwarder::addSourceSsrc(uint32_t ssrc) {
  if (rtcpData_.find(ssrc) == rtcpData_.end()) {
    this->rtcpData_[ssrc] = std::make_shared<RtcpData>();
...
void RtcpForwarder::analyzeSr(RtcpHeader* chead) {
  ...
  uint64_t now = wa::ClockUtils::timePointToMs(wa::clock::now());
  uint32_t ntp;
...
int RtcpForwarder::addNACK(char* buf, int len, uint16_t seqNum, uint16_t blp,
                           uint32_t sourceSsrc, uint32_t sinkSsrc) {
  buf += len;
