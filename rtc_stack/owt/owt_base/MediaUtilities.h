...
inline unsigned int calcBitrate(unsigned int width, 
                                unsigned int height, 
                                float framerate = 30) {
  ...
  int lines = sizeof(partial_linear_bitrate) / 
        sizeof(partial_linear_bitrate[0][0]) / 2;
  ...
  for (int i = 0; i < lines - 1; i++) {
    ...
    if (def > prev && def <= next) {
      ...
      bitrate = partial_linear_bitrate[i][1] + 
          (partial_linear_bitrate[i+1][1]-partial_linear_bitrate[i][1])*portion;
      break;
    }
  }
...
inline int findNALU(uint8_t* buf, 
                    int size, 
                    int* nal_start, 
                    int* nal_end, 
                    int* sc_len) {
