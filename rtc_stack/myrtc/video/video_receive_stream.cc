...
#include "video/vp8_header_parser.h"
#include "rtc_base/checks.h"
#include "rtc_base/keyframe_interval_settings.h"
#include "rtc_base/location.h"
#include "rtc_base/logging.h"
#include "rtc_base/string_builder.h"
#include "rtc_base/thread_registry.h"
...
VideoReceiveStream::VideoReceiveStream(
  ...
  for (const Decoder& decoder : config_.decoders) {
    ...
    RTC_CHECK(decoder_payload_types.find(decoder.payload_type) == decoder_payload_types.end())
  ...
  frame_buffer_.reset(
      new video_coding::FrameBuffer(clock_, timing_.get(), &stats_proxy_, &decode_queue_));
...
void VideoReceiveStream::Start() {
  ...
  rtc::VideoSinkInterface<VideoFrame>* renderer = this;
...
std::optional<Syncable::Info> VideoReceiveStream::GetInfo() const {
  RTC_DCHECK_RUN_ON(&module_process_sequence_checker_);
  std::optional<Syncable::Info> info =
