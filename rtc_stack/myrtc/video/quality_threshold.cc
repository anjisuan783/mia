...
std::optional<bool> QualityThreshold::IsHigh() const {
  return is_high_;
}

std::optional<double> QualityThreshold::CalculateVariance() const {
  if (until_full_ > 0) {
    return std::nullopt;
  }
...
std::optional<double> QualityThreshold::FractionHigh(
