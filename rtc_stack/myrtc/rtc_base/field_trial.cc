...
bool FieldTrialsStringIsValid(const std::string_view trials) {
  if (trials.empty())
    return true;

  size_t next_item = 0;
  std::map<std::string_view, std::string_view> field_trials;
  while (next_item < trials.length()) {
    ...
    if (group_name_end == trials.npos || name_end + 1 == group_name_end)
      return false;
    std::string_view name = trials.substr(next_item, name_end - next_item);
