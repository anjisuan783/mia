...
AsyncTCPSocketBase::AsyncTCPSocketBase(AsyncSocket* socket,
  ...
  if (!listen_) {
    ...
    inbuf_.EnsureCapacity(kMinimumRecvSize*10);
  }
...
// AsyncRawTCPSocket
AsyncRawTCPSocket* AsyncRawTCPSocket::Create(AsyncSocket* socket,
                                             const SocketAddress& bind_address,
                                             const SocketAddress& remote_address) {
  return new AsyncRawTCPSocket(
      AsyncTCPSocketBase::ConnectSocket(socket, bind_address, remote_address),
      false);
}
...
void AsyncRawTCPSocket::ProcessInput(char* data, size_t* len) {
  ...
  SignalReadPacket(this, data, *len, remote_addr, TimeMicros());

  *len = 0;
}

void AsyncRawTCPSocket::HandleIncomingConnection(AsyncSocket* socket) {
  SignalNewConnection(this, new AsyncRawTCPSocket(socket, false));
}
