...
std::optional<int> KeyframeIntervalSettings::MinKeyframeSendIntervalMs()
    const {
  return min_keyframe_send_interval_ms_.GetOptional();
}

std::optional<int> KeyframeIntervalSettings::MaxWaitForKeyframeMs() const {
  return max_wait_for_keyframe_ms_.GetOptional();
}

std::optional<int> KeyframeIntervalSettings::MaxWaitForFrameMs() const {
