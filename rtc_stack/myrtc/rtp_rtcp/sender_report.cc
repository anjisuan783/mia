...
DEFINE_LOGGER(SenderReport, "SenderReport")
...
bool SenderReport::Parse(const CommonHeader& packet) {
  ...
  if (packet.payload_size_bytes() <
    ...
    OLOG_WARN_THIS("Packet is too small to contain all the data.");
    return false;
  }
...
bool SenderReport::AddReportBlock(const ReportBlock& block) {
  if (report_blocks_.size() >= kMaxNumberOfReportBlocks) {
    OLOG_WARN_THIS("Max report blocks reached.");
    return false;
  }
...
bool SenderReport::SetReportBlocks(std::vector<ReportBlock> blocks) {
  if (blocks.size() > kMaxNumberOfReportBlocks) {
    OLOG_WARN_THIS("Too many report blocks (" << blocks.size()
                   << ") for sender report.");
    return false;
  }
