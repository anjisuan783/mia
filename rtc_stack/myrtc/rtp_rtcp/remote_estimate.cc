 ...
 public:
  ...
  bool Write(const NetworkStateEstimate& src, uint8_t* target) const {
    ...
    if (value.IsMinusInfinity()) {
      OLOG_WARN_THIS("Trying to serialize MinusInfinity");
      return false;
    }
    ...
    if (value.IsPlusInfinity()) {
      ...
      if (scaled >= kMaxEncoded) {
        scaled = kMaxEncoded;
        OLOG_WARN_THIS(ToString(value) << " is larger than max ("
...
DEFINE_LOGGER(DataRateSerializer, "DataRateSerializer");
