...
DEFINE_LOGGER(CommonHeader, "CommonHeader");
...
bool CommonHeader::Parse(const uint8_t* buffer, size_t size_bytes) {
  ...
  if (size_bytes < kHeaderSizeBytes) {
    OLOG_WARN("Too little data (" << size_bytes << " byte"
        << (size_bytes != 1 ? "s" : "")
  ...
  if (version != kVersion) {
    OLOG_WARN("Invalid RTCP header: Version must be "
  ...
  if (size_bytes < kHeaderSizeBytes + payload_size_) {
    OLOG_WARN("Buffer too small (" << size_bytes
  ...
  if (has_padding) {
    if (payload_size_ == 0) {
      OLOG_WARN("Invalid RTCP header: Padding bit set but 0 payload "
             "size specified.");
      return false;
    }
    ...
    if (padding_size_ == 0) {
      OLOG_WARN("Invalid RTCP header: Padding bit set but 0 padding "
             "size specified.");
      return false;
    }
    if (padding_size_ > payload_size_) {
      OLOG_WARN("Invalid RTCP header: Too many padding bytes ("
