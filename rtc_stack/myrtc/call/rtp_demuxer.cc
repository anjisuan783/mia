...
bool RtpDemuxer::AddSink(const RtpDemuxerCriteria& criteria, RtpPacketSinkInterface* sink) {
...
bool RtpDemuxer::CriteriaWouldConflict(const RtpDemuxerCriteria& criteria) const {
  if (!criteria.mid.empty()) {
    if (criteria.rsid.empty()) {
...
void RtpDemuxer::AddSink(const std::string& rsid, RtpPacketSinkInterface* sink) {
  RtpDemuxerCriteria criteria;
  criteria.rsid = rsid;
...
RtpPacketSinkInterface* RtpDemuxer::ResolveSink(const RtpPacketReceived& packet) {
...
bool RtpDemuxer::AddSsrcSinkBinding(uint32_t ssrc, RtpPacketSinkInterface* sink) {
