...
#include "rbe/remote_bitrate_estimator_single_stream.h"
...
#include "optional"
#include "rbe/aimd_rate_control.h"
#include "rbe/bwe_defines.h"
#include "rbe/inter_arrival.h"
#include "rbe/overuse_detector.h"
#include "rbe/overuse_estimator.h"
...
namespace webrtc {
namespace {
std::optional<DataRate> OptionalRateFromOptionalBps(
...
void RemoteBitrateEstimatorSingleStream::IncomingPacket(
  ...
  std::optional<uint32_t> incoming_bitrate = incoming_bitrate_.Rate(now_ms);
  if (incoming_bitrate) {
  ...
  if (estimator->detector.State() == BandwidthUsage::kBwOverusing) {
    std::optional<uint32_t> incoming_bitrate_bps =
