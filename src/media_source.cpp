...
bool MediaSource::initialize(std::shared_ptr<wa::Worker> worker, bool gop, bool atc) {
...
void MediaSource::on_publish() {
  is_monotonically_increase_ = true;
  active_ = true;
}

void MediaSource::on_unpublish() {
  active_ = false;
}
...
/*
void MediaSource::destroy_consumer(MediaConsumer* consumer) {
...
void MediaSource::on_audio_async(std::shared_ptr<MediaMessage> shared_audio) {
  RTC_DCHECK_RUN_ON(&thread_check_);
...
srs_error_t MediaSource::on_audio(std::shared_ptr<MediaMessage> shared_audio) {
  srs_error_t err = srs_success;
  ...
  async_task([shared_audio, this](std::shared_ptr<MediaSource> p) {
...
void MediaSource::on_video_async(std::shared_ptr<MediaMessage> shared_video) {
  RTC_DCHECK_RUN_ON(&thread_check_);
...
srs_error_t MediaSource::on_video(std::shared_ptr<MediaMessage> shared_video) {
  ...
  async_task([shared_video, this](std::shared_ptr<MediaSource> p) {
...
srs_error_t MediaSource::consumer_dumps(MediaConsumer* consumer, 
                                        ...
                                        bool dump_gop)
{
  ...
  if (active_) {
    ...
    if ((err = meta_->dumps(
          consumer, atc_, jitter_algorithm_, dump_meta, dump_seq_header)) != srs_success) {
      return srs_error_wrap(err, "meta dumps");
    }

    // copy gop cache to client.
    if (dump_gop && (err = gop_cache_->dump(consumer, atc_, jitter_algorithm_)) != srs_success) {
...
void MediaSource::async_task(std::function<void(std::shared_ptr<MediaSource>)> f) {
